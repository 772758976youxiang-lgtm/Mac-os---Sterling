...
模型发现会保留兼容列表显式给出的档位元数据。OpenRouter 风格的 `reasoning.supported_efforts` 会变成所选模型的 `reasoningEfforts`；`mandatory: true` 会阻止推断出“关闭”选项，而空值的受支持档位声明表示采用网关的完整档位词汇。列表未给出该字段时，精确的 `qwen3.7-flash` id 会采用内置的 `off` / `high` 修正，并配套使用 Qwen 的 `enable_thinking` 请求方言：Qwen Chat Completions 为这些模型可靠提供的是开关，而其通用 OpenAI 兼容层不会分派离散的 `reasoning_effort` 档位。若既没有结构化元数据，也没有 catalog 修正，发现结果就不声明档位：它不会用一次真实生成请求消耗额度，也不会从任意模型名猜测能力。
...
请求模态的解析顺序是：条目的 `input` → 已知模型能力修正 → 已安装 catalog 条目 → 路由的 `defaultInput`（默认 `[text]`），与上面两个容量字段的顺序和「回退值」定位完全一致。适配器目前内置了 `qwen3.7-flash` 的修正：即使已安装的 pi-ai catalog 尚未列出它，也会按同时接受文本和图片处理。显式的 `input` 仍然优先，因此部署可以收窄该模型。其他 catalog 模型和手动声明的模型继续保留 catalog 记录或路由声明的模态；而**未被 catalog 描述的**模型全都接受图片的网关，只需在路由上写一次 `[text, image]`，不必逐条目写。条目的空列表与缺省同义——它描述的是一个什么都不接受的模型，因此不作答，解析继续往下走——这正是当 `models` 条目点到某个 catalog 模型却不声明模态时，该模型仍保留 catalog 自有模态的原因。路由的那个则不得为空，因为它下面已经没有可以代为作答的层级。

模型页面的**获取可用模型**操作会保留兼容列表显式给出的输入模态元数据，包括常见的顶层 `input_modalities` 形式和 OpenRouter 的嵌套 `architecture.input_modalities`。选择候选模型时，这份声明会写入对应模型行。未披露这类元数据的列表仍保持自动：继续由上面的精确已知模型修正、已安装 catalog 和路由回退值决定。模型行的高级控件可显式选择**仅文本**或**文本 + 图片**；选择**自动**会移除按模型声明，重新采用这条解析链。

`[text]` 是「尚未声明」，而不是对端点的猜测——这也是为什么这里的最终回退值取保守值，而两个容量回退值只是取一个说得过去的值。许多 OpenAI 兼容模型列表只报告 id，而两种猜错的代价并不对等：模态中不含图片时，Harness 会在图片被附加之前就拒绝，因此少声明的代价是一次点名该模型的拒绝；而多声明会接纳一张图片、再由提供方在轮次中途拒绝——此时消息已经持久化，会话便会不断重复一个不可能成功的请求。
...
每次解析产出一份**不可变**快照——profiles 加上一个持有各聊天路由所建 `Provider` 的 `createModels()` 集合——每个操作都在自己第一个 `await` 之前整体捕获一份快照。配置变化会构造**新**集合，而不是改动正在被使用的那个：`Models.streamSimple()` 是惰性的，它在流首次被消费时才解析 provider，而那已在 credential await 之后，因此改动共享集合会让一个在旧配置下开始的请求在新配置下结束，或者撞上一个已不存在的 provider。这正是 seam 的每步调用冻结（`llm.prepareCall()`）能贯通到底的原因——回复途中切换模型会在下一步生效，绝不会影响在途的那一步。聊天请求经 `Models.streamSimple()` 抵达提供方；`openai-image-generations` 直接派发到 `/images/generations`，并返回助手图像附件。保持 catalog 协议不变的 catalog 路由会**复用**已安装提供方，只替换其模型列表，因为该提供方持有本包无法重建的 API 实现——Bedrock 经由独立入口加载其 Smithy 模块——从零件重建会静默收窄可用提供方的范围。其余聊天路由都由 `createProvider()` 基于 `supportedProtocols()` 背后的协议表构造，表中条目正是 pi-ai 自己的提供方工厂所用的同一批 factory。
