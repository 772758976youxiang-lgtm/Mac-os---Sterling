...
Select `openai-image-generations` for a route whose models implement OpenAI's Image API. The route declares text and image input by default, so attaching a reference image keeps the selected image model rather than invoking the host's vision-model fallback. A text-only message posts JSON to `/images/generations`; a message with one or more images reads their durable bytes and posts multipart `image[]` fields to `/images/edits`. Both send the latest direct-user text as `prompt`; an auxiliary request marked `purpose: 'image-generation'` may instead supply an explicit plugin prompt. Other plugin context never becomes the Image API prompt. Request-level `GenerateOptions.imageGeneration` values override configured `size`, `quality`, and `n` for that call; omitted values inherit the route, while `responseFormat` remains route-owned. The adapter saves every returned raster through the durable attachment service and emits image blocks. The defaults match the supplied invocation: `1024x1024`, `high`, one image, and `b64_json`.

```yaml
providers:
  rayplus-images:
    displayName: RayPlus Images
    apiKeyEnv: RAYPLUS_API_KEY
    ...
    imageGeneration:
      ...
      quality: high
      n: 1
      responseFormat: b64_json # use url when the endpoint does not return b64_json
```
...
Model discovery preserves explicit effort metadata from compatible listings. OpenRouter-style `reasoning.supported_efforts` becomes the selected model's `reasoningEfforts`; `mandatory: true` suppresses an inferred Off option, while a nullable supported-efforts declaration represents the gateway's full effort vocabulary. The exact `qwen3.7-flash` ids receive a curated `off` / `high` correction when a listing omits the field, paired with Qwen's `enable_thinking` request dialect: Qwen Chat Completions exposes a reliable off/on toggle for these models, while its generic OpenAI compatibility layer does not dispatch discrete `reasoning_effort` levels. Listings with neither structured metadata nor a catalog correction remain undeclared: discovery does not spend quota on an active generation probe or guess from an arbitrary model name.
...
Request modalities resolve entry `input` → known model capability corrections → installed catalog entry → route `defaultInput` (default `[text]`), the same order and the same fallback role the capacities above use. The adapter currently carries a curated correction for `qwen3.7-flash`, which accepts text and images even when the installed pi-ai catalog does not list it yet. An explicit `input` still wins, so a deployment can narrow that model. Other catalog and hand-declared models keep the modalities the catalog records or the route declares; a gateway whose *undescribed* models all take images declares `[text, image]` once at the route instead of on every entry. An entry's empty list means the same as an absent one — it describes a model accepting nothing, so it states no answer and resolution continues past it, which is what keeps a catalog model's own modalities when a `models` entry names it without declaring any. The route's may not be empty, since nothing sits below it to answer instead.

The Models page's **Fetch available models** action preserves explicit input-modality metadata from compatible listings, including the common top-level `input_modalities` form and OpenRouter's nested `architecture.input_modalities`. Selecting a candidate writes that declaration into its model row. Listings that disclose no such metadata remain automatic: the exact known-model correction, installed catalog, and route fallback above still decide them. The row's Advanced controls can explicitly choose **Text only** or **Text + image**, while **Automatic** removes the per-model declaration and returns to that resolution chain.

`[text]` is the absence of a declaration rather than a guess at the endpoint, which is why the final fallback here is conservative where the capacity fallbacks are merely plausible. Many OpenAI-compatible listings report ids only, and the two wrong answers do not cost the same: the harness refuses an image before it is attached when a model's modalities omit one, so under-claiming costs a refusal naming the model, while over-claiming admits an image the provider then rejects mid-turn — after the message is durable, which leaves the session repeating a request that cannot succeed.
...
Each resolution produces one **immutable** snapshot — the profiles plus a `createModels()` collection holding the `Provider` each chat route built — and every operation captures a whole snapshot before its first `await`. A configuration change builds a *new* collection rather than mutating the one in use: `Models.streamSimple()` resolves its provider lazily, when the stream is first consumed, which is after the credential await, so a mutated collection would let a request that started under one configuration finish under another or fail on a provider that no longer exists. This is what makes the seam's per-step call freeze (`llm.prepareCall()`) hold end to end — switching models mid-reply takes effect on the next step, never inside the one in flight. Chat requests reach their provider through `Models.streamSimple()`; `openai-image-generations` is dispatched directly to `/images/generations` and returns assistant image attachments. A catalog route that keeps its catalog protocol **reuses** the installed provider with its model list replaced, because that provider owns API implementations this package cannot reconstruct — Bedrock loads its Smithy module through a separate entry point — so rebuilding it from parts would silently narrow which providers work. Every other chat route is built by `createProvider()` over the protocol table behind `supportedProtocols()`, whose entries are the same factories pi-ai's own provider factories use.
