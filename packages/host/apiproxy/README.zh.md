...
`session.attachment` 以 base64 返回一张经会话授权的图片。授权既识别持久消息内容中的完整图片引用（包括嵌套的 tool-result block），也识别 `tool/result` 展示元数据 `images` 数组中的完整图片引用。元数据路径会在访问存储前校验完整引用形态；任意其他元数据和字段不完整的仿冒对象都不会授予访问权。附件存储随后还会依据该已记录引用校验内容寻址对象。
