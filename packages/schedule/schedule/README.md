...
`ScheduleService` owns that queue and exposes the same create, list, and delete operations to Host callers with camelCase rule fields. Its update operation preserves event-log immutability by appending delete for the active id and create for a newly allocated id in one managed transaction, followed by one persistence barrier for the combined append. Model tools remain adapters over this service and keep their existing snake_case protocol.
