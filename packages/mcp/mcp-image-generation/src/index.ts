...
/** Default Qwen vision model used by the multimodal fallback. */
export const DEFAULT_VISION_MODEL = 'qwen3.7-flash'
/** Default OpenAI-compatible DashScope endpoint for Qwen vision. */
export const DEFAULT_VISION_BASE_URL = 'https://dashscope.aliyuncs.com/compatible-mode/v1'
/** Default credential reference used by the Qwen vision fallback. */
export const DEFAULT_VISION_API_KEY_ENV = 'DASHSCOPE_API_KEY'
/** Public MCP-style tool name available to text models. */
...
/** The tool uses the existing model router and durable attachment store. */
...
export const Config: z<Config> = z.object({
  ...
  visionApiKeyEnv: z.string().role('credential-ref').default(DEFAULT_VISION_API_KEY_ENV),
...
/** Resolve a vision key at request time so rotation applies without restart. */
async function resolveVisionApiKey(ctx: Context, config: Required<Config>): Promise<string | undefined> {
  // Config is resolved from defaults at runtime, but the explicit key remains optional by design.
  // oxlint-disable-next-line typescript/no-unnecessary-condition
  if (config.visionApiKey !== undefined && config.visionApiKey.length > 0) return config.visionApiKey
...
/** Call an OpenAI-compatible Qwen vision endpoint with durable image bytes. */
async function analyzeWithQwen(input: {
  ...
  const response = await fetch(visionEndpoint(input.baseURL), {
    method: 'POST',
    headers: {
      ...
      messages: [{
        ...
        content: [
          ...
          ...input.images.map(image => ({
            type: 'image_url',
            image_url: { url: visionDataUrl(image.data, image.mediaType) },
          })),
        ],
      }],
    }),
  ...
  if (!response.ok) {
    ...
    throw new Error(`Qwen vision request failed (${response.status})${detail}`)
  }
  ...
  throw new Error('Qwen vision returned no text')
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Return textual understanding of images attached to the latest human message with Qwen3.7-Flash. Use only for description, OCR, comparison, extraction, or visual questions. Do not use this as preparation for image generation or editing; mcp__image__generate_image receives source images directly.',
    parameters: {
    ...
    async execute(args, exec): Promise<VisionValue> {
      ...
      if (apiKey === undefined) throw new Error(`Qwen vision has no API key for "${active.visionApiKeyEnv}"; configure it in MCP 管理`)
      ...
      if (refs.length === 0) throw new Error('Qwen vision requires an image in the latest human message')
      ...
      const text = await analyzeWithQwen({
        baseURL: active.visionBaseURL,
      ...
      return { model: active.visionModel, images: images.length, text }
    },
    presentCall(args) {
