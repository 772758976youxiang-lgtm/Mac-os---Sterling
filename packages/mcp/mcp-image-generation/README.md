...
- id: mcp-image-generation
  name: '@deepseek-ai/dsh-mcp-image-generation'
  config:
    imageProvider: acme-images # custom Image API provider (endpoint required)
    imageDisplayName: Acme Images
    imageBaseURL: https://gateway.example/v1
    imageApi: openai-image-generations
    imageApiKeyEnv: ACME_IMAGES_API_KEY
    model: image-gen-2
```
