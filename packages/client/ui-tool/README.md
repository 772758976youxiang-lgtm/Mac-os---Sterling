...
This package currently owns the generic fallback and the built-in shell/pwsh, read, write/edit, grep/glob, web, image-generation, todo, question, and Code Dispatch presentations. The image-generation view replaces a successful call row with its durable attachment gallery; running, failed, and malformed results retain the row for progress and diagnostics. `ui-skill` demonstrates a business-owned registration for `skill`.
