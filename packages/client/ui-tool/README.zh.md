...
本包当前拥有 generic fallback，以及 shell/pwsh、read、write/edit、grep/glob、web、image-generation、todo、question 和 Code Dispatch 的内置展示。image-generation 视图会用持久附件图片组取代成功调用行；执行中、失败和元数据异常的结果仍保留工具行，用于显示进度与诊断信息。`ui-skill` 展示了业务包自行拥有的 `skill` 注册项。
