...
`ProducedFiles` 在收尾消息正文与其 IconActions 之间渲染该行：一个低调的标签，以及本轮产出路径构成的纵向标签列表——每个文件一行，全部显示，不折叠，也没有 `+ N 个文件` 的剩余计数。每个标签项经由属主提供的 `openFile` 打开——与工具行相同的 Host 打开器，chat 视图会把相对路径按会话 cwd 解析。
