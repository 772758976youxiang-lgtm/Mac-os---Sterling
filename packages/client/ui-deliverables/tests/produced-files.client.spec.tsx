...
describe('ProducedFiles row', () => {
  ...
  it('renders every produced path as its own openable chip, never folded', () => {
    ...
    // No cap and no remainder count: every path gets a chip.
    expect(within(row).getAllByRole('button')).toHaveLength(paths.length)
    expect(view.queryByText(/^\+ \d+ 个文件$/)).toBeNull()
    for (const path of paths) {
      const chip = view.getByRole('button', { name: `打开 ${path}` })
      expect(chip.textContent).toBe(basename(path))
      expect(chip.getAttribute('title')).toBe(path)
      fireEvent.click(chip)
    }
    expect(openFile.mock.calls.map(call => call[0])).toEqual(paths)
  })

  it('renders an empty row with no chips when nothing was produced', () => {
    const view = render(<ProducedFiles matched={[]} openFile={() => {}} t={t} />)
    expect(view.getByText('产物')).toBeTruthy()
    const row = view.container.querySelector('[data-produced-files-row]')
    if (!(row instanceof HTMLElement)) throw new Error('produced row missing')
...
describe('plugin registration', () => {
  it('registers the tail entry and fiber disposal removes it', async () => {
    ...
    // locale's own inject declaration waits on 'connection' even though the
    // deliverables row no longer reads it; the stub keeps the bench alive.
    ...
    // The row injects nothing: all data arrives through owner props and the locale seat.
