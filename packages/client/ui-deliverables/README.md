...
`ProducedFiles` renders the row between the closing message's body and its IconActions footer: a quiet label and the turn's produced paths as stacked chips, one per row, every path shown — no folding, no `+ N files` remainder. Each chip opens through the owner-supplied `openFile` — the same Host opener the tool rows use, with the chat view resolving relative paths against the session cwd.
