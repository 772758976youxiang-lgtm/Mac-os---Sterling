/** Browser-side staged settings and credential controller for a custom image-generation provider. */
...
/** Protocol choices shared with the image-generation provider form. */
...
const DEFAULT_API = IMAGE_API_PROTOCOLS[0]
const DEFAULT_API_KEY_REF = 'STERLING_IMAGE_API_KEY'

/** Settings fields owned by the local image-generation plugin. */
...
export interface ImageSettingsFace {
  hooks: {
    imageSettings: SnapshotStore<ImageSettingsState>
  }
...
export class ImageSettingsController {
  private readonly store: SnapshotStore<ImageSettingsState>
  ...
  constructor(
    private readonly scope: SettingsScope<ImageSettings>,
  ...
  private value(field: keyof ImageSettings): string {
    const snapshot = this.scope.getSnapshot().value
    const staged = field === 'imageProvider' ? this.providerDraft
      : field === 'imageDisplayName' ? this.displayNameDraft
        : field === 'imageBaseURL' ? this.baseURLDraft
          : field === 'imageApi' ? this.apiDraft
            : field === 'model' ? this.modelDraft
              : undefined
    if (staged !== undefined) return staged
    const current = snapshot?.[field]
    if (typeof current === 'string') return current
    return field === 'imageApi' ? DEFAULT_API : ''
  }

  private apiKeyRef(): string {
    const stored = this.scope.getSnapshot().value?.imageApiKeyEnv?.trim()
    return stored || providerKeyRef(this.value('imageProvider'))
  }

  private valid(): boolean {
    // The provider id is a label for the direct Image API dispatch, not a
    // route key, so any non-empty id is acceptable.
    return this.value('imageProvider').trim() !== ''
      && this.value('imageBaseURL').trim() !== ''
      && validBaseURL(this.value('imageBaseURL'))
      && this.value('model').trim() !== ''
      && IMAGE_API_PROTOCOLS.includes(this.value('imageApi') as typeof IMAGE_API_PROTOCOLS[number])
  }

  /** Build the view from the current scope and staged form values. */
  private projection(): ImageSettingsState {
    ...
    return {
      ...
      provider: this.value('imageProvider'),
      displayName: this.value('imageDisplayName'),
      baseURL: this.value('imageBaseURL'),
      api: this.value('imageApi'),
  ...
  save(): void {
    ...
    void (async () => {
      try {
        ...
        const values: Array<[keyof ImageSettings, string]> = [
          ['imageProvider', this.value('imageProvider')],
          ['imageDisplayName', this.value('imageDisplayName')],
          ['imageBaseURL', normalizedBaseURL(this.value('imageBaseURL'))],
          ['imageApi', this.value('imageApi')],
          ['model', this.value('model')],
        ]
        for (const [field, value] of values) {
          if (value !== (current?.[field] ?? (field === 'imageApi' ? DEFAULT_API : ''))) {
  ...
  inject(): ImageSettingsFace {
    return {
      hooks: { imageSettings: this.store },
...
export function unavailableImageSettingsFace(): ImageSettingsFace {
  const state: ImageSettingsState = {
    available: false,
    writable: false,
    saving: false,
    failed: false,
  ...
  return {
    hooks: { imageSettings: createSnapshotStore(state) },
    edit: () => {},
    refresh: () => {},
    save: () => {},
    discard: () => {},
