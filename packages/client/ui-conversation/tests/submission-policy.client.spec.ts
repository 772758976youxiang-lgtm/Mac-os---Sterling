...
describe('ComposerSubmissionPolicy', () => {
  ...
  it('adopts a Host preference without writing it back and leaves an identical write untouched', () => {
    ...
    host.publish({
      status: 'ready', value: { busyEnter: 'steer', showContextInjections: true, showToolCalls: true, showThinking: true }, revision: 1, writable: true,
    })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
    policy.setBusyEnter('steer')
    expect(host.set).not.toHaveBeenCalled()
    host.publish({ value: { busyEnter: 'steer', showContextInjections: true, showToolCalls: true, showThinking: true }, revision: 2 })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
  })
