 ...
 * pass through. Only the shipped modes are localized — an unknown
 * host-configured name has no translation, so it keeps the transform.
 */
...
/** Localized label for a shipped permission option; unknown modes keep the transform. */
function optionLabel(option: PermissionSelectValue['options'][number], t: ComposerBarProps['t']): string {
  ...
  if (option.value === 'read-only') return t('permission.readOnly')
  if (option.value === 'workspace-write') return t('permission.workspaceWrite')
  return displayName(option.name)
}

/** Localized label for a raw permission value (fallback when the option is absent). */
function valueLabel(value: string, t: ComposerBarProps['t']): string {
  if (value === FULL_ACCESS) return t('permission.fullAccess')
      ...
      <Menu
        ...
        anchor={
          <button
            ...
            aria-label={t('input.accessMode', { name: current === undefined ? valueLabel(currentValue, t) : optionLabel(current, t) })}
          ...
          >
            ...
            <span className={css.triggerLabel}>{current === undefined ? valueLabel(currentValue, t) : optionLabel(current, t)}</span>
