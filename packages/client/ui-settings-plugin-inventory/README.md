...
Read-only **Plugin list** and **MCP management** tabs for Web Settings. The browser plugin registers localized `settings.plugins.tab` contributions with ids `all` and `mcp`; the Plugins section owns the navigation entry and tab chrome. It performs no Remote read during plugin activation. Selecting either tab for the first time mounts it and lazily calls `ctx.remote.pluginInventory.list()` or `ctx.remote.pluginInventory.mcp()` through [`api-remotes`](../../api/remotes/README.md).
...
MCP management uses the same compact, searchable disclosure pattern. It groups currently registered MCP tools under their configured server namespace, shows the safe transport kind, lifecycle status, and tool count, and reveals public tool names plus server-provided descriptions when expanded. Refreshing replaces only this local snapshot. It never displays URLs, process commands, headers, environment values, or other connection secrets.
...
- **MCP is a current capability view** — it lists tool capabilities only after discovery registered them; it cannot add, edit, restart, or remove MCP servers.
