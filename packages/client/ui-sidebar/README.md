...
During a live collapse, the shell holds the expanded content at its current width while it fades out for 150ms. The upper controls then share one 150ms fade and 49px leftward translation into the 56px rail, ending with the layout's 300ms column slide; every 36px control box follows the same path to the rail's 10px left inset. The shell toggle and optional brand actions occupy separate vertical rows so neither can be clipped by the rail, followed by New Session plus add and search rendered through `sidebar.workspaces`. The bottom-pinned `sidebar.settings` control shares the fade timing but has no horizontal translation. A page that starts collapsed renders the rail statically, and reduced-motion mode disables both transitions.
