...
`ui-conversation` also registers a root-scoped `shell.overlay` completion banner. It observes the same live session-list running projection, seeds every known session without an announcement, and displays `Task completed` for two seconds only after a later running→idle edge. The banner covers both the selected session and background scheduled followups, replaces an earlier banner when another session completes, and keeps no durable or manager-owned notification state.
...
- **Using the sidebar reminder for the banner.** Rejected because it excludes the selected session and disappears when the user opens the session; the banner must announce every live completion without changing the sidebar's unread semantics.
