...
[`dsh-web-app`](../../../../packages/bundle/web-app/README.md) 组合包默认加载 `@deepseek-ai/dsh-time-context` 与 `@deepseek-ai/dsh-schedule`。Schedule 只观察插件加载后发布的根 Agent，并在该 Agent scope 中安装三个工具和一个可丢弃 owner。cold history 读取、已发布的根、child Agent 与其他 host 都不会激活它。
