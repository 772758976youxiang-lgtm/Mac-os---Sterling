...
The [`dsh-web-app`](../../../../packages/bundle/web-app/README.md) bundle loads `@deepseek-ai/dsh-time-context` and `@deepseek-ai/dsh-schedule` by default. Schedule observes only root Agents published after the plugin loads and installs its three tools plus one disposable owner in that Agent scope. Cold history reads, already-published roots, child Agents, and other hosts do not activate it.
