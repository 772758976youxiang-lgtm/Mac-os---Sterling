...
[![License: MIT](https://img.shields.io/github/license/772758976youxiang-lgtm/Sterling-Harness-win--)](https://github.com/772758976youxiang-lgtm/Sterling-Harness-win--/blob/main/LICENSE)
[![Release](https://img.shields.io/github/v/release/772758976youxiang-lgtm/Sterling-Harness-win--)](https://github.com/772758976youxiang-lgtm/Sterling-Harness-win--/releases)
[![Last commit](https://img.shields.io/github/last-commit/772758976youxiang-lgtm/Sterling-Harness-win--)](https://github.com/772758976youxiang-lgtm/Sterling-Harness-win--)
[![CI](https://img.shields.io/github/actions/workflow/status/772758976youxiang-lgtm/Sterling-Harness-win--/ci.yml)](https://github.com/772758976youxiang-lgtm/Sterling-Harness-win--/actions)
...
- **Multi-provider live balance** — Selects DeepSeek `/user/balance` or Rayplus `/v1/usage` from the active model route and uses that route's own credential. It refreshes every 60 seconds and retains the last successful snapshot per route. Other gateways can declare a custom adapter; unsupported gateways are reported explicitly and never receive an invented balance.
...
- **API key**: the bar reuses the active route's credential reference from **Settings → Models**. DeepSeek defaults to `DEEPSEEK_API_KEY`; Rayplus and other custom routes use their own `apiKeyEnv`, so keys are never duplicated into plugin configuration.
- **Balance adapters**: DeepSeek and `rayplus.site` are detected automatically. For another gateway, set `DSH_BOTTOM_INFO_BAR_BALANCE_ADAPTERS` in the launch environment to a JSON object keyed by provider route, for example `{"my-route":{"endpoint":"https://billing.example/account","credential":"MY_API_KEY","parser":"paths","balancePath":"account.remaining","currency":"USD"}}`. Parser kinds are `deepseek`, `rayplus`, and `paths`; a `paths` adapter may also use `currencyPath`. A route without a known or declared billing API displays an unsupported message instead of a fabricated balance.
