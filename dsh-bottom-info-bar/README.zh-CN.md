...
- **多平台实时余额**：按当前模型路由自动选择 DeepSeek `/user/balance` 或 Rayplus `/v1/usage`，每条路由使用自己配置的凭据；60 秒自动刷新，失败保留该路由上次快照。其他平台可声明自定义余额适配器，不支持余额 API 时明确提示且不伪造余额
...
- **API Key**：信息栏复用 **设置 → 模型** 中当前路由的凭据引用。DeepSeek 默认使用 `DEEPSEEK_API_KEY`；Rayplus 等自定义路由使用各自的 `apiKeyEnv`，无需重复保存密钥。
- **余额适配器**：DeepSeek 与 `rayplus.site` 自动识别。其他平台可在启动环境中设置 `DSH_BOTTOM_INFO_BAR_BALANCE_ADAPTERS`，值为按 provider 路由键索引的 JSON。例如 `{"my-route":{"endpoint":"https://billing.example/account","credential":"MY_API_KEY","parser":"paths","balancePath":"account.remaining","currency":"USD"}}`。`parser` 可选 `deepseek`、`rayplus`、`paths`；`paths` 还可用 `currencyPath` 从响应读取币种。未声明且没有内置适配器的平台显示「该平台暂不支持余额查询」。
...
| 余额显示「未配置 …」 | 在 设置 → 模型 配置当前路由提示的凭据 |
| 显示「该平台暂不支持余额查询」 | 该平台没有内置余额接口；按「余额适配器」配置自定义端点，或只使用原生统计信息 |
