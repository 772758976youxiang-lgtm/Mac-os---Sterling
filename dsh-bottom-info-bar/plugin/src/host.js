...
// ---------- 余额提供方适配（只解析非 secret 配置；credential 始终按引用交给 credentials seam） ----------
...
function parseCustomBalanceAdapters(raw) {
  ...
  for (const [provider, entry] of Object.entries(source)) {
    if (!provider || !entry || typeof entry !== 'object' || Array.isArray(entry)) continue
    if (typeof entry.endpoint !== 'string' || typeof entry.credential !== 'string') continue
    if (!['deepseek', 'rayplus', 'paths'].includes(entry.parser)) continue
    try {
    ...
    if (entry.parser === 'paths' && (typeof entry.balancePath !== 'string' || entry.balancePath.length === 0)) continue
    out[provider] = {
      endpoint: entry.endpoint,
      credential: entry.credential,
      parser: entry.parser,
      ...(entry.balancePath ? { balancePath: entry.balancePath } : {}),
      ...(entry.currencyPath ? { currencyPath: entry.currencyPath } : {}),
...
function balanceProfileForRoute(providerId, settings) {
  if (!providerId || !settings || typeof settings !== 'object') return null
  if (providerId === 'deepseek' || providerId.startsWith('deepseek-')) {
    const deepseek = settings['llm-deepseek']
    return deepseek && typeof deepseek === 'object' ? deepseek : null
  }
  const piAi = settings['llm-pi-ai']
  const providers = piAi && typeof piAi === 'object' ? piAi.providers : null
  const profile = providers && typeof providers === 'object' ? providers[providerId] : null
  return profile && typeof profile === 'object' ? profile : null
}

function normalizeBalanceEndpoint(baseURL, path) {
  try {
...
function resolveBalanceAdapter(providerId, profile, customAdapters) {
  if (!providerId || SUBSCRIPTION_PROVIDERS.includes(providerId)) return null
  const custom = customAdapters && customAdapters[providerId]
  if (custom) {
    return {
      kind: 'custom',
      ...custom,
      displayName: profile && profile.displayName ? profile.displayName : providerId,
    }
  }
  const baseURL = profile && typeof profile.baseURL === 'string' ? profile.baseURL : ''
  let host = ''
  try { host = baseURL ? new URL(baseURL).hostname.toLowerCase() : '' } catch (err) { /* unsupported below */ }
  if (providerId === 'deepseek' || providerId.startsWith('deepseek-') || host === 'api.deepseek.com') {
    return {
      kind: 'deepseek',
      endpoint: normalizeBalanceEndpoint(baseURL || 'https://api.deepseek.com', '/user/balance'),
      credential: profile && profile.apiKeyEnv ? profile.apiKeyEnv : 'DEEPSEEK_API_KEY',
      parser: 'deepseek',
      displayName: profile && profile.displayName ? profile.displayName : 'DeepSeek',
    }
  }
  if ((host === 'rayplus.site' || host.endsWith('.rayplus.site')) && profile && profile.apiKeyEnv) {
    return {
      kind: 'rayplus',
      endpoint: normalizeBalanceEndpoint(baseURL, '/v1/usage'),
...
function parseBalanceResponse(adapter, body) {
  if (!adapter || !body || typeof body !== 'object') return null
  let value
  let currency
  let granted = 0
  let toppedUp
  if (adapter.parser === 'deepseek') {
    const list = Array.isArray(body.balance_infos) ? body.balance_infos : []
    const record = list.find(function (entry) { return entry && entry.currency === 'CNY' }) || list[0]
    if (!record) return null
    value = record.total_balance
    currency = record.currency || 'CNY'
    granted = Number(record.granted_balance) || 0
    toppedUp = Number(record.topped_up_balance)
  } else if (adapter.parser === 'rayplus') {
    value = body.remaining !== undefined ? body.remaining : body.balance
    currency = body.unit || body.currency || 'USD'
  } else if (adapter.parser === 'paths') {
    value = readPath(body, adapter.balancePath)
    currency = adapter.currencyPath ? readPath(body, adapter.currencyPath) : adapter.currency
  } else {
    return null
  }
  ...
  return {
    ...
    total: total,
    granted: granted,
    toppedUp: Number.isFinite(toppedUp) ? toppedUp : total,
  }
}

function mergeBalanceResult(previous, result, fetchedAt) {
  if (!result || result.error) {
    return {
      data: previous && previous.data ? previous.data : null,
      fetchedAt: previous && previous.fetchedAt ? previous.fetchedAt : null,
      error: result && result.error ? result.error : { kind: 'exception', message: '余额请求未知异常' },
    }
  }
  return { data: result.data || null, fetchedAt: fetchedAt, error: null }
}

const CUSTOM_BALANCE_ADAPTERS = parseCustomBalanceAdapters(process.env.DSH_BOTTOM_INFO_BAR_BALANCE_ADAPTERS)
...
export default {
  ...
  apply(ctx) {
    ...
    let config = {
      ...
      activeProvider: 'deepseek-official',
    ...
    function balanceSettingsSnapshot() {
      const settings = ctx.get ? ctx.get('settings') : null
      if (!settings || typeof settings.get !== 'function') return {}
      return {
        'llm-deepseek': settings.get('llm-deepseek'),
        'llm-pi-ai': settings.get('llm-pi-ai'),
      }
    }

    function balanceAdapterForRoute(pid) {
      const settings = balanceSettingsSnapshot()
      return resolveBalanceAdapter(pid, balanceProfileForRoute(pid, settings), CUSTOM_BALANCE_ADAPTERS)
    }

    function activeProviderId() {
      const selected = modelSelection().provider
      return selected || config.activeProvider
    }

    async function refreshProviderBalance(pid) {
      if (!pid || detectBillingMode(pid, config.billingMode).mode === 'subscription') return
      const adapter = balanceAdapterForRoute(pid)
      if (!adapter) {
        balances[pid] = mergeBalanceResult(balances[pid], {
          error: { kind: 'unsupported', message: '当前平台未提供余额查询接口' },
        }, Date.now())
        return
      }
      ...
      let cred = null
      try {
        cred = await ctx.credentials.resolve(adapter.credential)
      } catch (err) {
        if (balanceSeq[pid] === seq) balances[pid] = mergeBalanceResult(balances[pid], { error: { kind: 'credentials', message: '凭据读取失败' } }, Date.now())
        return
      }
      if (!cred || !cred.value) {
        if (balanceSeq[pid] === seq) balances[pid] = mergeBalanceResult(balances[pid], { error: { kind: 'no-key', message: '未配置 ' + adapter.credential, credential: adapter.credential } }, Date.now())
        return
      }
      try {
        // API Key 只经 HTTP 头传递，不进入日志、持久化记录或子进程命令行。
        const res = await fetch(adapter.endpoint, {
          headers: { Authorization: 'Bearer ' + cred.value },
          signal: AbortSignal.timeout(15000),
        })
        if (!res.ok) {
          if (balanceSeq[pid] === seq) balances[pid] = mergeBalanceResult(balances[pid], { error: { kind: 'http', message: '请求失败（HTTP ' + res.status + '）' } }, Date.now())
          return
        }
        const parsed = parseBalanceResponse(adapter, await res.json())
        if (!parsed) {
          if (balanceSeq[pid] === seq) balances[pid] = mergeBalanceResult(balances[pid], { error: { kind: 'parse', message: '响应格式异常' } }, Date.now())
          return
        }
        if (balanceSeq[pid] === seq) balances[pid] = mergeBalanceResult(balances[pid], { data: parsed }, Date.now())
      } catch (err) {
        if (balanceSeq[pid] === seq) balances[pid] = mergeBalanceResult(balances[pid], { error: { kind: 'exception', message: String((err && err.message) || err) } }, Date.now())
      }
    }

    function refreshAllBalances() {
      return refreshProviderBalance(activeProviderId())
    }

    function ensureProviderBalance(pid) {
      const snap = balances[pid]
      if (snap && snap.data && snap.error === null && Date.now() - snap.fetchedAt < 60000) return Promise.resolve()
      return refreshProviderBalance(pid)
    }
    ...
    function activeBalanceSummary(providerId, nowMs) {
      ...
      const adapter = balanceAdapterForRoute(pid)
      const profile = balanceProfileForRoute(pid, balanceSettingsSnapshot())
      ...
      return {
        provider: pid,
        displayName: adapter ? adapter.displayName : (profile && profile.displayName ? profile.displayName : pid),
        estimate: false,
        currency: snap.data ? snap.data.currency : (adapter && adapter.kind === 'deepseek' ? 'CNY' : 'USD'),
        data: snap.data,
    ...
    function activeCurrency() {
      const snap = balances[activeProviderId()];
    ...
    function spendSummary(nowMs) {
      const snap = balances[activeProviderId()] || { data: null };
    ...
    function computeEstimate(nowMs) {
      ...
      const bal = activeBalanceSummary(pid, nowMs);
      ...
      return {
        ...
        pricing: pricing,
        fetchedAt: balances[pid] ? balances[pid].fetchedAt : null,
        stale: !!balances[pid] && balances[pid].error !== null && balances[pid].data !== null,
        error: balances[pid] ? balances[pid].error : null,
      };
    }
    ...
    function providerList(nowMs) {
      ...
      if (settings['llm-deepseek']) ids.add(active.startsWith('deepseek') ? active : 'deepseek-official')
      const piAi = settings['llm-pi-ai']
      const profiles = piAi && piAi.providers && typeof piAi.providers === 'object' ? piAi.providers : {}
      for (const pid of Object.keys(profiles)) ids.add(pid)
      return [...ids].map(function (pid) {
        const summary = activeBalanceSummary(pid, nowMs)
        return {
          id: pid,
          ...
          total: summary.data ? summary.data.total : null,
    ...
    const ROUTES = {
      getBalanceSnapshot: async function (args) {
        const requested = args && typeof args === 'object' && args.provider ? String(args.provider) : ''
        const pid = requested || activeProviderId()
        await ensureProviderBalance(pid)
        return activeBalanceSummary(pid, Date.now())
      },
      ...
      getProviders: function () {
        return { providers: providerList(Date.now()), activeProvider: activeProviderId() };
      },
      setActiveProvider: function (args) {
        ...
        return { activeProvider: activeProviderId() };
      },
      ...
      getConfig: function () {
        return { displayMode: config.displayMode, infoDensity: config.infoDensity, activeProvider: activeProviderId(), alertThreshold: config.alertThreshold, billingMode: config.billingMode };
      },
