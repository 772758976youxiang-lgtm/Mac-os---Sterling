    ...
    function BottomInfoBar(props) {
      ...
      function pushBalanceGroups(groups) {
        ...
        if (bal && bal.error && bal.error.kind === 'no-key') {
          const credential = bal.error.credential || 'API Key';
          groups.push(React.createElement('span', { className: 'bi-err', key: 'nokey' },
            '未配置 ' + credential + ' → 设置→模型 填写'));
        } else if (bal && bal.error && bal.error.kind === 'unsupported') {
          groups.push(React.createElement('span', { className: 'bi-stale', key: 'unsupported' },
            '该平台暂不支持余额查询'));
