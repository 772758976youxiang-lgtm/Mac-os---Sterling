...
function makeStub(providerId, model, options = {}) {
  const captured = { llmListener: null, route: null, intervalCalls: 0, credentialRefs: [] }
  const ctx = {
    get(name) {
      ...
      if (name === 'settings') {
        return { get: (ns) => options.settings && options.settings[ns] }
      }
      return undefined
    },
    credentials: {
      resolve: async (ref) => {
        captured.credentialRefs.push(String(ref))
        const value = options.credentials && options.credentials[String(ref)]
...
// ---------- 多平台余额：当前路由决定端点与凭据，不回退其他服务商 ----------
{
  const originalFetch = globalThis.fetch
  const fetchCalls = []
  globalThis.fetch = async (url, init) => {
    fetchCalls.push({ url: String(url), authorization: init && init.headers && init.headers.Authorization })
    return { ok: true, status: 200, json: async () => ({ unit: 'USD', balance: 451.8, remaining: 450.5 }) }
  }
  try {
    const settings = {
      'llm-pi-ai': { providers: { a: { displayName: 'ChatGPT', baseURL: 'https://rayplus.site', apiKeyEnv: 'A_API_KEY' } } },
      'llm-deepseek': {},
    }
    const rayplus = makeStub('a', 'gpt-5.5', { settings, credentials: { A_API_KEY: 'rayplus-test-key' } })
    const dispose = plugin.apply(rayplus.ctx)
    await new Promise((resolve) => setTimeout(resolve, 30))
    const r = await invoke(rayplus.captured.route, '/_dsh/dsh-bottom-info-bar/getBalanceSnapshot', 'GET')
    check('Rayplus 当前路由 → 自己的 USD 余额', r.status === 200 && r.payload.provider === 'a' && r.payload.currency === 'USD' && r.payload.data && r.payload.data.total === 450.5)
    check('Rayplus 当前路由 → /v1/usage + A_API_KEY', fetchCalls.length === 1 && fetchCalls[0].url === 'https://rayplus.site/v1/usage' && fetchCalls[0].authorization === 'Bearer rayplus-test-key')
    check('Rayplus 当前路由不读取 DeepSeek 凭据', JSON.stringify(rayplus.captured.credentialRefs) === JSON.stringify(['A_API_KEY']))
    dispose()

    const unknown = makeStub('unknown', 'mystery', {
      settings: { 'llm-pi-ai': { providers: { unknown: { baseURL: 'https://unknown.example/v1', apiKeyEnv: 'UNKNOWN_KEY' } } } },
    })
    const disposeUnknown = plugin.apply(unknown.ctx)
    await new Promise((resolve) => setTimeout(resolve, 30))
    const unsupported = await invoke(unknown.captured.route, '/_dsh/dsh-bottom-info-bar/getBalanceSnapshot', 'GET')
    check('未知平台明确 unsupported，不回退 DeepSeek', unsupported.status === 200 && unsupported.payload.provider === 'unknown' && unsupported.payload.error && unsupported.payload.error.kind === 'unsupported')
    check('未知平台不读取任何凭据', unknown.captured.credentialRefs.length === 0)
    disposeUnknown()
  } finally {
...
// ---------- 订阅制模式（Codex / OpenCode Go provider；auth 文件被隔离为不存在 → no-key，不发网络） ----------
{
  ...
  check('Codex 订阅路由不读取余额凭据', subCtx.captured.credentialRefs.length === 0)
  {
