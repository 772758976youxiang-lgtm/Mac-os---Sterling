...
describe('web e2e: plugin configuration section', () => {
  ...
  it('shows one card per exposed host-plane namespace', async () => {
    ...
    // agent loop, the DeepSeek search provider, and the optional email delivery.
    ...
    expect(await dialog.getByText('邮件发送', { exact: true }).count()).toBe(1)
    expect(await dialog.getByRole('tab', { name: 'MCP 管理', exact: true }).count()).toBe(1)
